...
This notebook aims to compare gene expression obtained from PacBio data unfiltered of any artefacts and Illumina data.
...
pdf("Figures_2025_RevisionsNAR/Figure4C.pdf", width=7, height=18)
op = par(mfrow=c(5,2), mar = c(3,4.5,2,2))
final_toCounts = NULL
n = 1
...
### Overlap in the detected genes from common cells {.tabset}
...
pdf("Figures_2025_RevisionsNAR/Figure4B.pdf", width=6, height=5)
ggplot() + 
  geom_bar(data = melt(shareddf[c(1,3,4)]), aes(Sample, value, fill = variable), position = "dodge", stat = "identity", show.legend = TRUE) + 
  geom_bar(data=melt(shareddf[c(1,2)]), stat="identity", aes(Sample, value, fill = variable), show.legend = TRUE) +
  scale_fill_manual(name = "", values = myColors, labels=c("Illumina","PacBio","Common")) +
...
pdf("Figures_2025_RevisionsNAR/Figure4A.pdf", width=6, height=5)
...
Finding genes with significantly divergent counts between PacBio and Illumina.
...
pdf("Figures_2025_RevisionsNAR/Figure4D.pdf", width=6, height=5)
...
## Cell clustering analysis on merged data {.tabset}
...
pdf("Figures_2025_RevisionsNAR/Figure4E.pdf", height=15, width=7)
...
pdf("Figures_2025_RevisionsNAR/Figure4F.pdf", height=3, width=5)
...
pdf("Figures_2025_RevisionsNAR/Figure4G.pdf", width=8, height=6)
