...
#Read in cell annotations from Tülay
...
1. How many isoforms and reads are filtered
...
merge(
    ...
    facet_wrap(~variable, scales = "free_y") + 
    scale_color_manual(values = myColors) + 
    theme(axis.text.x = element_text(angle = 45, hjust= 1, vjust = 1, size = 12), axis.text.y = element_text(size = 12), strip.text = element_text(size = 13), legend.text = element_text(size = 12)) + 
    labs(x = "", y = "percentage")
```


2. The so called artefacts what do they classify as
...
2. What do the alignments look like in Illumina 

How many are counted, how many are unspliced
...
melt(x[,-7], id.vars = c("Sample", "label", "tagIds")) %>% 
    ...
    geom_boxplot() + labs(x = "label", y = "percentage", color = "alignment type") + theme(legend.position = "top") + labs(x = "Matching Illumina Reads")
```
...
3. Low coverage and NonCanonical have the biggest impact  - correlation


If you compare to nonfiltered data:
...
4. Which genes are affected? What do we expect from those genes?
...
p1 = comb[,c(1,6,9)] %>% ggplot(aes(Sample, featWidth, fill = EdgeRLabel)) + geom_violin() + scale_fill_manual(values = c("#A6761D", "#440154FF", "cyan")) + labs(y = "Gene Length", fill = "") + theme( legend.text = element_text(size = 11), legend.position = "top", legend.spacing.x = unit(3, 'cm'), text = element_text(size = 13), axis.text.x = element_text(size = 13)) + ylim(0,6000) + stat_summary(fun.y=mean, position=position_dodge(width=0.9), colour = "red")
p1
```


5. Length of the reads that map to those genes
...
General length of filtered artefacts
...
ggplot(reasons_pbAnnot_df[reasons_pbAnnot_df$filter %in% c("Unfiltered", "LowCoverage/Non-Canonical"),], aes(length, color = filter2)) + geom_density() + scale_color_manual(values = c("red", "black")) + theme(strip.text = element_text(size = 15), legend.position = "top", legend.text = element_text(size = 17)) + xlim(0,3500) + labs(color="PacBio isoform category", x = "Read Length",y = "Density") + theme(text = element_text(size = 14), axis.text.y = element_text(size = 17), axis.text.x = element_text(size = 17)) + facet_grid(~Sample)
```
...
6. Lets compare PacBio to PacBio unfiltered  - pseudobulk correlation
...
for (i in names(pacbio_pseudobulk)){
    ...
    #sf <- ezLogmeanScalingFactor(totCounts, presentFlag = totCounts >0)
    xNorm <- totCounts
    ezXYScatter(xNorm[ , "PacBio"]+10, xNorm[ ,  "PacBio_unfiltered"]+ 10, xlab = "PacBio", ylab = "PacBio_unfiltered", types = gcTypes, main = i, cex = 1, cex.lab = 2, cex.main = 2)
    ...
    ezXYScatter(xNorm[ , "PacBio"]+10, xNorm[ ,  "PacBio_unfiltered"]+ 10, xlab = "PacBio", ylab = "PacBio_unfiltered", types = widthTypes, main = i, cex = 1, cex.lab =2, cex.main = 2)
    legend(x='topright', legend=paste('R=', round(x$estimate,4)), cex = 1.5)
    xNorm = xNorm %>% rownames_to_column()
...
7. Which genes are affected? What do we expect from those genes?
...
cluster3_sample3 <- FindMarkers(i, ident.1 = "5_Illumina", ident.2 = "4_PacBio", verbose = FALSE)
